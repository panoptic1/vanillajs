...
/**
 * Render data from cache in the event of API failure
 */
var renderFallback = function(){
    ...
    //if there's nothing saved, render fail message in the UI (last resort)
    if (!saved) {
        renderNoArticles();
        return;
    }
    ...
    console.log(`Fallback from cache, boy.`)
};
/**
...
var fetchArticles = function () {
	...
	fetch('https://vanillajsacademy.com/api/pirate.json').then(function (response) {
		if (response.ok) {
